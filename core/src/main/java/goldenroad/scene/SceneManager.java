...
public class SceneManager {
    ...
    private static final String ROOM1_FILE = "room1.json";
    ...
    private void setupPrototypeData() {
        ScreenData map1 = createMapOne();
        ScreenData map2 = createMapTwo();
        ScreenData map3 = createMapThree();
    ...
    private ScreenData createMapOne() {
        ScreenData tiledMap = loadTiledScreen(
            ROOM1_FILE,
            List.of(
                new GroundMonster(1000, 720, 44, 42, color(220, 70, 70), 8, 112, 760, 2040, 2.0, 8.8, 1, 8, color(255, 145, 120), RenderShape.OVAL)
            ),
            List.of(
                new Item(560, 560, 18, 18, color(80, 210, 120), Item.Shape.OVAL)
            )
        );
        if (tiledMap != null) {
            return tiledMap;
    ...
    private ScreenData loadTiledScreen(String mapFileName, List<Monster> monsters, List<Item> items) {
        String json = readTextFromAssets(mapFileName);
    ...
    private String readTextFromAssets(String assetPath) {
    ...
    private ScreenData createMapTwo() {
        ...
        return buildScreenFromGrid(
            grid,
            List.of(
                new GroundMonster(760, 640, 42, 40, color(230, 85, 65), 8, 120, 100, 2360, 2.2, 9.0, 1, 8, color(255, 150, 125), RenderShape.TRIANGLE),
                new AirMonster(1560, 300, 38, 36, color(95, 185, 230), 7, 148, 1040, 2460, 80, 420, 2.5, 10.0, 1, 8, color(145, 220, 255), RenderShape.RECTANGLE)
            ),
            List.of(
                new Item(1490, 220, 16, 16, color(245, 205, 90), Item.Shape.OVAL)
            )
        );
    }

    private ScreenData createMapThree() {
        ...
        return buildScreenFromGrid(
            grid,
            List.of(
                new GroundMonster(430, 600, 44, 40, color(210, 75, 75), 9, 124, 80, 2550, 1.9, 9.4, 1, 8, color(255, 150, 135), RenderShape.RECTANGLE),
                new GroundMonster(2340, 720, 36, 36, color(175, 80, 230), 8, 120, 1980, 2500, 2.0, 9.2, 1, 8, color(240, 180, 255), RenderShape.TRIANGLE),
                new AirMonster(1840, 320, 40, 36, color(100, 190, 235), 7, 156, 1440, 2520, 120, 520, 2.4, 10.2, 1, 8, color(150, 230, 255), RenderShape.OVAL)
            ),
            List.of(
                new Item(1050, 460, 16, 16, color(105, 220, 170), Item.Shape.OVAL),
                new Item(2460, 680, 18, 18, color(250, 205, 95), Item.Shape.RECTANGLE)
            )
        );
    }

    private ScreenData buildScreenFromGrid(char[][] grid, List<Monster> monsters, List<Item> items) {
        List<Rectangle> solidBlocks = new ArrayList<>();
